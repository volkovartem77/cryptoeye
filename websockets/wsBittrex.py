...
from utils_log import log, err_log
...
class BittrexWebsocket:
    ...
    def __on_message(self, message):
        ...
        if "M" in message and len(message['M']) > 0:
            ...
            if 'A' in message and len(message['A']) > 0:
                ...
                set_tick(self.__NAME, self.__symbol, json.dumps({
                    'bid': bid,
                    'ask': ask,
    ...
    def __on_error(self, error):
        log(f'ws{self.__NAME}: {error}')

    def __on_close(self):
        log(f'ws{self.__NAME}: {"closed"}')

    def __on_open(self):
        log(f'ws{self.__NAME} subscribe {self.__symbol}')
